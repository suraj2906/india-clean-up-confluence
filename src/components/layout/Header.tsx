  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        scrolled || open
          ? "border-b border-forest/10 bg-shell/85 backdrop-blur-md"
      ...
      <div className="container-page flex h-20 items-center justify-between">
        ...
        <nav aria-label="Primary" className="hidden items-center gap-1 lg:flex">
          {nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-full px-4 py-2 text-sm font-medium text-forest/80 transition-colors hover:bg-forest/5 hover:text-forest"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="rounded-full p-2.5 text-forest transition hover:bg-forest/5 lg:hidden"
          >
      ...
      <AnimatePresence>
        {open && (
          <motion.nav
            id="mobile-nav"
            aria-label="Mobile"
            className="overflow-hidden border-t border-forest/10 bg-shell lg:hidden"
            ...
            >
              {nav.map((item, i) => (
                ...
                >
                  <Link
                    href={item.href}
                    className="block rounded-xl px-4 py-3 font-display text-xl text-forest transition hover:bg-leaf-100"
                  >
                    {item.label}
                  </Link>
                </motion.li>
