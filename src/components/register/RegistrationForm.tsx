...
import { AlertCircle, CheckCircle2, Loader2, Megaphone, Send, Trophy, Users } from "lucide-react";
...
const { oneCeo } = registration;

/** One per `oneCeo.steps`, in the same order. */
const stepIcons = [Users, Trophy, Megaphone];

/**
 * The one registration form, for everyone. Same client-side path as
 * `ContactForm` — a direct POST to Web3Forms, no server handler, same access key
 * and therefore the same inbox (see the contact-form note in AGENTS.md).
 *
 * It carries a second form inside it, and the reveal happens in two stages.
 * Choosing "NGO or clean-up movement" in the dropdown opens the One CEO, Many
 * Missions explanation — what the session is and how the ten are picked — right
 * above the tick box, because that is the only moment we know the reader is one
 * of the people it is for. Ticking the box then opens the application questions,
 * which are rendered straight out of `registration.oneCeo.questions` in
 * `site.ts`; the component knows nothing about what is being asked.
 *
 ...
 *
 * While `questions` is empty (it is: they are still being written) ticking the
 * box shows `oneCeo.pending` instead and the registration still submits, flagged
 * as a pitch applicant. That is deliberate — the page can be live and collecting
 * names before anyone has decided what the panel needs to know.
 */
export function RegistrationForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (pitching) {
      for (const q of oneCeo.questions) {
    ...
    try {
      const res = await fetch(ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          subject: pitching
            ? `${site.name} 3.0 registration — ${attendee} — One CEO, Many Missions`
          ...
          one_ceo_many_missions: pitching ? "Yes — wants to pitch" : "No",
          ...answers,
      ...
      if (res.ok && json.success) {
        setStatus("sent");
        form.reset();
        // `form.reset()` returns the uncontrolled fields to their defaults but
        // not these two, which React owns.
        setPitching(false);
        setAttendee(registration.attendeeTypes[0]);
      } else {
        setStatus("error");
        setMessage(
          json.message ?? `Something went wrong. Please email us at ${contact.email} instead.`,
        );
      }
    } catch {
      setStatus("error");
  ...
  return (
    ...
    >
      ...
      {/* The One CEO, Many Missions track. Closed by default: most people
      ...
      <div className="mt-7 rounded-3xl border border-sky-700/20 bg-sky-50 p-5 sm:p-6">
          ...
          {showPitchInfo && (
            ...
            >
              <div className="mb-5 border-b border-sky-700/15 pb-5">
                ...
                <h3 className="mt-2 text-lg font-semibold sm:text-xl">{oneCeo.title}</h3>

                <div className="mt-3 space-y-3 text-sm leading-relaxed text-muted">
                  {oneCeo.body.map((paragraph) => (
                    <p key={paragraph.slice(0, 24)}>{paragraph}</p>
                  ))}
                </div>

                <ol className="mt-5 grid gap-4 sm:grid-cols-3">
                  {oneCeo.steps.map((step, i) => {
        ...
        <label className="flex cursor-pointer items-start gap-3">
          ...
          />
          <span>
            <span className="block text-sm font-semibold text-ink">{oneCeo.question}</span>
            <span className="mt-1 block text-xs leading-relaxed text-muted">{oneCeo.hint}</span>
          </span>
        </label>

        {pitching && (
          <div className="mt-6 border-t border-sky-700/15 pt-6">
            {oneCeo.questions.length === 0 ? (
              <p className="text-sm leading-relaxed text-muted">{oneCeo.pending}</p>
            ) : (
              <div className="grid gap-5">
                {oneCeo.questions.map((q) => (
                  <Field
                    key={q.name}
                    label={q.label}
                    name={q.name}
